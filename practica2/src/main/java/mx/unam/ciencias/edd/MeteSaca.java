...
public abstract class MeteSaca<T> {
    ...
    public T saca() throws NoSuchElementException {
        // Aquí va su código.
      if(this.cabeza == null) throw new NoSuchElementException();
    ...
    @Override public boolean equals(Object object) {
      ...
      while(!(m.esVacia()) && !(this.esVacia())){
        try{
          if(m.saca() != this.saca()) return false;
        } catch(Exception e){
          System.out.println(e.getMessage());
        }
      }
      if(!m.esVacia() || !this.esVacia()) return false;
      return true;
        // Aquí va su código.
    }
}
